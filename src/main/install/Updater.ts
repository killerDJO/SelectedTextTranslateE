...
export class Updater {
    constructor(
        ...
        autoUpdater.on("update-not-available", () => this.notificationSender.send("No updates available.", "Click to visit releases page.", () => shell.openExternal(updateSettings.releasesUrl)));
        autoUpdater.on("update-available", () => this.notificationSender.send("Update available.", "Update is downloading in background..."));
        autoUpdater.on("update-downloaded", (_, releaseNotes, releaseName) => this.promptForUpdate(releaseNotes, releaseName));
